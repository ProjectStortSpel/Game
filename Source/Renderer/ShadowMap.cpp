...
ShadowMap::ShadowMap(vec3 lightPos, vec3 target, int res)
{
	...
	m_resolution = res;
	m_projectionMatrix = glm::ortho(-10.0, 10.0, -10.0, 10.0, 1.5, 100.0); 
...
void ShadowMap::SetBounds(float _width, float _height)
{
	m_width = _width;
	m_height = _height;
	m_projectionMatrix = glm::ortho(-_width, _width, -_height, _height, 1.5f, 100.f + std::max(_width, _height));
}
