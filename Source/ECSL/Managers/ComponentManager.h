...
namespace ECSL
{
	...
	public:
		...
		static TypeId GetTableId(std::string componentType)
		{
			...
			auto it = m_stringTableId.find(componentType);
			if (it == m_stringTableId.end())
			{
				TypeId newId = ++ComponentManager::m_nextTableId;
				m_stringTableId.insert(std::pair<std::string, TypeId>(componentType, newId));
				return newId;
			}
			else
				return it->second;
