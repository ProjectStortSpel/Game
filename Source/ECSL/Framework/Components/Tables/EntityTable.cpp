...
void EntityTable::AddComponentTo(unsigned int _entityId, unsigned int _componentTypeId)
{
	...
	/* The entity is dead */
	assert(!(*(unsigned char*)m_dataTable->GetData(_entityId) == 0) || DataLogger::GetInstance().WriteToLogAssert());

	/* The component is already added to the entity */
	assert(!(entityComponents[bitSetIndex] & ((BitSet::DataType)1 << bitIndex)) || DataLogger::GetInstance().WriteToLogAssert());
...
unsigned int EntityTable::GenerateNewEntityId()
{
	...
	assert(m_availableEntityIds->size() != 0 || DataLogger::GetInstance().WriteToLogAssert());
...
void EntityTable::AddOldEntityId(unsigned int _entityId)
{
	...
	assert(_entityId < m_entityCount || DataLogger::GetInstance().WriteToLogAssert());
