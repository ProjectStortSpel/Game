...
namespace LuaEmbedder
{
  ...
  const char* EXPORT Load(const std::string& filepath);
  const char* EXPORT CallFunction(const std::string& name, int argumentCount = 0, const std::string& library = std::string());
