...
MapSystem.OnEntityAdded = function(self, entity)
	...
	--local mapComp = world:GetComponent(entity, "Rotation", 0)
	...
	world:GetComponent(mapEntity, "MapSize", 0):SetInt2(self.mapX, self.mapY)
			...
			if waterA ~= waterB then
				...
				if posAX + dirAX == posBX and posAY + dirAY == posBY then
					if dirAX ~= dirBX and dirAY ~= dirBY then
						...
						local comp = world:GetComponent(waterTiles[waterB], "Model", 0)
						...
						local comp = world:GetComponent(waterTiles[waterB], "Rotation", 0)
...
MapSystem.AddTile = function(self, posX, posZ, tiletype)
    ...
    local posComp = world:GetComponent(entity, "Position", 0)
    posComp:SetFloat3(posX, 0.0, posZ)
	
    local mapPosComp = world:GetComponent(entity, "MapPosition", 0)
    ...
    if tiletype == 104 then -- 104 = h = hole
		...
		local rotComp = world:GetComponent(entity, "Rotation", 0)
		rotComp:SetFloat3(0, math.pi * 0.5 * math.random(0, 4), 0)
    ...
    elseif tiletype >= 49 and tiletype <= 57 then -- 49 = 1 = first checkpoint, 57 = 9 = 9th checkpoint
        ...
        local comp = world:GetComponent(entity, "Checkpoint", 0)
    ...
    elseif tiletype == 117 then -- 117 = u = water up
        ...
        local comp = world:GetComponent(entity, "River", 0)
        comp:SetInt3(0, -1, 1)
		local comp = world:GetComponent(entity, "Rotation", 0)
...
MapSystem.AddGroundTileBelow = function(self, posX, posZ)
	...
	local posComp = world:GetComponent(groundEntity, "Position", 0)
	posComp:SetFloat3(posX, 0.0, posZ)
	
	local mapPosComp = world:GetComponent(groundEntity, "MapPosition", 0)
	mapPosComp:SetInt2(posX, posZ)
	
	world:CreateComponentAndAddTo("Model", groundEntity)
	local comp = world:GetComponent(groundEntity, "Model", 0)
	comp:SetModel("grass", "grass", 0)
end